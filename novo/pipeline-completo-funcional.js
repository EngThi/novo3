...
class PipelineCompletoFuncional {
    constructor() {
        ...
        console.log('🚀 Pipeline Completo Funcional V6.0');
    ...
    async gerarScript(topic) {
        ...
        const prompt = `🚀 PIPELINE COMPLETO V6.0 - GEMINI 2.5 FLASH
...
Crie roteiro completo para vídeo de 3 minutos:

ESTRUTURA:
1. ABERTURA (0-20s): Gancho + data
2. DESENVOLVIMENTO (20s-2min20s): 4-5 pontos principais
3. CONCLUSÃO (2min20s-3min): Resumo + CTA
    ...
    async gerarImagemPollinations(prompt, jobId, sceneIndex) {
        ...
        try {
            ...
            if (response.data && response.data.byteLength > 5000) {
                ...
                console.log(`✅ Pollinations: ${filename}`);
...
tts.save("${audioPath}")
print("✅ Narração gTTS gerada")
                `]);
                
                gtts.on('close', (code) => {
                    if (code === 0) {
                        console.log(`✅ gTTS: ${audioFilename}`);
                        resolve();
                    } else {
            ...
            console.log(`⚠️ gTTS falhou, usando espeak: ${error.message}`);
    ...
    async montarVideoFinal(script, imagens, audioPath, jobId) {
        ...
        console.log(`🎬 Montando vídeo com ${imagens.length} imagens...`);
        ...
        await new Promise((resolve, reject) => {
            ...
            ffmpeg.stderr.on('data', (data) => {
                ...
                if (output.includes('time=')) {
                    ...
                    if (timeMatch) {
                        process.stdout.write(`\r🎬 Processando: ${timeMatch[1]}`);
                    }
                }
            });
            ...
            ffmpeg.on('close', (code) => {
                console.log(`\n`);
                if (code === 0) {
                    console.log(`✅ Vídeo final: ${videoFilename}`);
    ...
    createFallback() {
        ...
        return {
            title: `Análise Completa - ${currentDate}`,
            content: `Bem-vindos ao nosso canal! Hoje, ${currentDate}, vamos analisar os temas mais importantes. Este é um roteiro completo com análise detalhada dos principais pontos. Fiquem conosco até o final para descobrir insights exclusivos. Não se esqueçam de curtir o vídeo e se inscrever no canal!`,
            hook: `Descubra os temas mais importantes de hoje, ${currentDate}`,
            keyPoints: ["Análise principal", "Pontos importantes", "Conclusões", "Próximos passos"],
            duration: "180",
            seoKeywords: ["analise", currentDate.replace(/\//g, ''), "completa"],
            callToAction: "Curta, se inscreva e ative as notificações!"
        };
    }
    async gerarNarracaoPremium(content, jobId) {
        try {
            ...
            console.log(`✅ TTS Premium: ${resultado.filename} (${resultado.fileSizeKB}KB, ${resultado.duration})`);
            ...
            // Fallback para espeak
            return this.gerarNarracao(content, jobId);
        }
    }
...
async function main() {
    if (process.argv.length < 3) {
        console.log('📋 Uso: node pipeline-completo-funcional.js "seu tópico aqui"');
        process.exit(1);
    }
